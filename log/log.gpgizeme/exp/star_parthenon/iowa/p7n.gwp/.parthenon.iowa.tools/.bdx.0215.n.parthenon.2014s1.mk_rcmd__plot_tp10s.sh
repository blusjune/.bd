...
#_taglist="xfs ext4 f2fs";
_taglist=""
_taglist_tmp=$(echo $_tgt_2s | sed -e 's/-/_/g');
for _i in $_taglist_tmp; do
	_tag_trim=$(echo $_i | awk -F'_' '{ print $2 }');
	_taglist="$_taglist $_tag_trim";
done

_taglist_wcw=$(echo $_taglist | wc -w);
...
echo "
par(mfcol=c($_dlist_a_wcw, $_taglist_wcw));
";
_df_list="";
_name_list="";
_ca=0;
for _ia in $_taglist; do
	_ca=$(expr $_ca + 1);
	_cb=0;
