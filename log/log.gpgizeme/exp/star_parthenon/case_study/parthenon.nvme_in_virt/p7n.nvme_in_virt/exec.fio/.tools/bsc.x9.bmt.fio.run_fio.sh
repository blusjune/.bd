...
if [ "X$NTHREADS" = "X" ]; then
	read -p "#>> {NTHREADS} number of threads: " NTHREADS;
	if [ "X$NTHREADS" = "X" ]; then
		echo "#>> ERROR 'NTHREADS' variable should be specified -- EXIT 1";
		exit 1;
	fi
fi
...
#  $FIO --output=$Testcase --name=$Testcase --filename=$DEV --ioengine=posixaio --direct=1 --norandommap --randrepeat=0 --refill_buffers --time_based --ramp_time=30s --runtime=30s --blocksize=$4 --rw=$2 --rwmixwrite=$3 --iodepth=$6 --overwrite=1 --numjobs=$5 --group_reporting
  $FIO --output=$Testcase --name=$Testcase --filename=$DEV --ioengine=posixaio --direct=1 --norandommap --randrepeat=1 --refill_buffers --time_based --ramp_time=30s --runtime=40s --blocksize=$4 --rw=$2 --rwmixwrite=$3 --iodepth=$6 --overwrite=0 --numjobs=$5 --group_reporting
...
#  local RequestSize="1k 4k 8k 16k 128k 4096k"
  local RequestSize="4k"
#  local QueueDepth="1 2 4 8 16 32 64 128 256"
  local QueueDepth="6 12 24 48 96"
  local Job=$1
  local BlockAlign=$2
...
fio_exec_pt()
{
	RandomRun4kA_blusjune "$NTHREADS";	# 24-threads (!!-Dell Requirement-!! need long time)
	#RandomRun4kA_blusjune "1";	# 1-threads (!!-Dell Requirement-!! need long time)
