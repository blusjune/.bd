...
_tdir="tdir/iowa.sim.iox.${_tstamp}.sigio_${_ioc_percent}.iowsz_${_iow_size}.t1_${_prd_t1}";
if [ ! -d $_tdir ]; then
	mkdir -p $_tdir;
fi
...
_tstamp_begin=".job_begin__$(tstamp)"; touch $_tdir/$_tstamp_begin; echo "$_tstamp_begin";
cat infile | bsc.iowa.lsp.sim.iox -c $_ioc_percent -w $_iow_size -p $_prd_t1 > $_tdir/$_logfile
_tstamp_finish=".job_finish__$(tstamp)"; touch $_tdir/$_tstamp_finish; echo "$_tstamp_finish";
