#!/bin/sh
##.bdx.0100.y.linux_kernel_build.sh
##_ver=20121212_222042


_tstamp="$(tstamp)";
_whoami="$(whoami)";
_build_dir__default="/x/k.build/lkb-$_tstamp";	# LKB: Linux Kernel Build
...
if [ ! -d $_build_dir ]; then
	...
	if [ ! -f ${_build_dir}/${_config_latest} ]; then
		cp $_config_latest $_build_dir;
		echo "#>> latest .config file ($_config_latest) is copied to $_build_dir";
	else
		echo "#>> existing .config file will be used ($_build_dir/$_config_latest)";
	fi
fi
